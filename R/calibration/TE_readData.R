...
obs_sl      = dat[,2]/1000              # data are in mm
obs_sl_time = dat[,1]-0.5 # they give half-year so it is unambiguous as to which year they mean
obs_sl_err  = dat[,3]/1000
...
for (i in 1:nrow(trends.te)) {
  idx = compute_indices(obs.time=(trends.te[i,4]:trends.te[i,5]) , mod.time=mod_time)
